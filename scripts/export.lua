...
local function doExport()
  ...
  global.export_data = {
    current = 1,
    chunks = slices,
    save_mode = settings.global["graftorio-ng-server-save"].value or false
  }

  -- clear out file
  if global.export_data.save_mode then
    game.write_file("graftorio-ng/game.prom", "", false, 0)
  else
    game.write_file("graftorio-ng/game.prom", "", false)
  end
end
    ...
    [defines.events.on_tick] = function(event)
      ...
      if d and d.current and d.chunks[d.current] and event.tick % 4 == 0 and not translate.in_progress() then
        ...
        if table_size(result) > 0 then
          if d.save_mode then
            game.write_file("graftorio-ng/game.prom", table.concat(result, "\n") .. "\n", true, 0)
          else
            game.write_file("graftorio-ng/game.prom", table.concat(result, "\n") .. "\n", true)
          end
        end
